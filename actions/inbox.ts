...
// プラン別SLAは plans テーブル（/admin/plans で編集可）から読む。
// ここにハードコードのマップを置くと、管理画面で変更しても受信トレイだけ
// 古い値のままになる（保存も表示も成功するため気づけない）。lib/plan-sla.ts 参照。
...
export async function getInboxItems(
  ...
  // ===== 受信トレイに必要な per-user 集計を1本のRPCで取得 =====
  // 最新メッセージ・最終受信/送信・未読数・本日送信数・最新チェックイン・
  // 未解決リスク・過去リスク有無を DB 側で集計する（旧: messages/checkins/risk_flags を
  // 個別クエリ＆一部は全件取得していたのを1回のRPCに集約。転送量・往復・JS処理を削減）。
  // SLA設定は1リクエストにつき1回だけ読む（ユーザーごとに引かない）
  const [{ data: threadSummaries }, planSlaMap] = await Promise.all([
    supabase.rpc("inbox_thread_summary", {
      p_user_ids: userIds,
      p_staff_id: staff.id,
      p_today_start: todayStart.toISOString(),
    }),
    loadPlanSlaMap(supabase),
  ]);

  const summaryMap = new Map((threadSummaries ?? []).map((s) => [s.end_user_id, s]));
  ...
  const items: InboxItem[] = users.map((user) => {
    ...
    // 受信トレイに必要な値はすべて DB 集計（RPC）から取得する。
    ...
    const lastCheckinDateStr = summary?.last_checkin_date ?? null; // 最新チェックイン日(yyyy-mm-dd)
    const hasOpenRisk = summary?.has_open_risk ?? false;
    const openRiskLevel = summary?.open_risk_level ?? null;
    const hadPreviousRisk = summary?.had_previous_risk ?? false;
    const todaySentCount = summary?.today_sent_count ?? 0;
    const unreadCount = summary?.unread_count ?? 0;

    const planCode = user.plan_code as "light" | "standard" | "premium";
    const slaConfig = planSlaMap.get(planCode);
    ...
    // 未報告判定
    const lastCheckinDate = lastCheckinDateStr
      ? new Date(lastCheckinDateStr + "T00:00:00")
      : null;
