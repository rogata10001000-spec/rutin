...
import { z } from "zod";
import { inviteStaff } from "@/actions/admin/staff";
...
export function InviteStaffDialog({ open, onClose }: InviteStaffDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result.ok) {
        showToast(`${data.displayName}さんに招待メールを送信しました`, "success");
        onClose();
        router.refresh();
      } else {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-1 flex-col overflow-hidden">
            <div className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
              ...
              <div>
                ...
                {errors.role && (
                  <p className="mt-1.5 text-sm text-red-600 font-medium">{errors.role.message}</p>
                )}
              </div>
