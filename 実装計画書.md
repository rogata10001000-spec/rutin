...
### Issue 08：Inbox一覧
- **目的**：優先度・未返信が見える。ユーザー返信をリアルタイム反映
- **変更候補**：`app/(admin)/inbox/*`, `actions/inbox.ts`, `components/inbox/InboxAutoRefresh.tsx`
- **実装ステップ**
  1. Inboxデータ取得
  ...
  4. paused優先度低下
  5. フィルタ/表示
  6. Supabase Realtime 連動（`InboxAutoRefresh`）+ 30秒ポーリングフォールバック
- **完了条件（DoD）**：SLA警告表示、未報告が上位、pausedが下位、新着返信で即時更新
...
### Issue 10：チャット履歴・送信
- **目的**：返信・代理返信の基盤。新着メッセージのリアルタイム表示
- **変更候補**：`app/(admin)/chat/*`, `actions/messages.ts`, `components/chat/ChatContainer.tsx`, `lib/message-realtime.ts`
- **実装ステップ**
  1. 履歴表示（SSR初回）
  2. 送信/代理返信（楽観的更新）
  3. Supabase Realtime で新着追記
- **完了条件（DoD）**：送信が記録される。開いたまま新着 in/out が表示される
- **テスト観点**：E2E-040/041
...
### Issue 24：リアルタイム表示・Web Push通知
- **目的**：管理画面の即時更新とスタッフ向け通知（Safari 含む）
- **変更候補**：`lib/message-realtime.ts`, `hooks/useMessageRealtime.ts`, `components/common/*`, `lib/push-notifications.ts`, `public/sw.js`, `supabase/migrations/20260521100000_enable_messages_realtime.sql`
- **実装ステップ**
  1. `messages` を Supabase Realtime publication に追加
  2. 共有 Realtime 購読基盤 + Inbox/Chat 連動
  3. Web Push（VAPID + SW + `staff_push_subscriptions`）
  4. 担当 cast + admin/supervisor への Push 送信
  5. Safari PWA 対応（manifest / apple-touch-icon / iOS 案内 UI）
- **完了条件（DoD）**：Inbox/Chat リアルタイム更新、Web Push 受信、iOS PWA 案内表示
- **テスト観点**：UT-020/021、E2E-043〜045（手動）
- **リスクとロールバック方法**：VAPID 未設定時は Push スキップ（UI は「未設定」表示）
...
- Web Push 通知は全スタッフが有効化可能（cast / admin / supervisor）
- iOS Safari では PWA（ホーム画面追加）が Web Push の前提
